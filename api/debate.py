...
def handle_history(room_id: str) -> Dict:
    """Handle GET /api/debate/history"""
    init_db()
...
def handle_step(room_id: str, user_prompt: Optional[str] = None) -> Dict:
    """Handle POST /api/debate/step"""
    init_db()
    ...
    if user_prompt:
        ...
        # Note: we don't include user message in new_messages because the frontend adds it optimistically
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        """Handle GET requests (history endpoint)"""
        parsed_url = urlparse(self.path)

        if parsed_url.path == "/api/debate/history":
    ...
    def do_POST(self):
        """Handle POST requests (step endpoint)"""
        if self.path == "/api/debate/step":
            ...
            try:
                ...
                result = handle_step(room_id, user_prompt)
